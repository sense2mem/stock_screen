...
# SELL（出口）シグナル設定
# =========================
SELL_SCORE_MIN = 4          # 売り抽出の最低点（まずは 4 推奨。多すぎる→5、少なすぎる→3）
TRAIL_STOP_20D_PCT = 0.08   # 直近20日高値から -8% でトレーリングストップ
EXT_DEV200_MAX = 0.18       # 200MA乖離 +18% 以上は利確優先サイン（過熱）
RSI_OVERBOUGHT = 70
RSI_EXTREME = 78
ADX_SELL_MIN = 20
NATR_SPIKE_MULT = 1.4       # NATRが60日平均の1.4倍超なら“荒れ”
SELL_EXIT_BEFORE_EARNINGS = True
EARNINGS_EXIT_PRE_BDAYS = 1 # 決算の何営業日前までに逃げるか（0-1が無難）
...
def score_buy_signals(df_daily):
    """
    既存の買いスコア（score）を維持しつつ、
    追加で sell_score / sell_reason / sell_フラグ群 を返す（sell版）。
    """
    ...
    if len(r.dropna()) >= 2:
        ...
        if np.isfinite(r0) and np.isfinite(r1):
            sell_rsi_reversal = bool((r0 >= RSI_OVERBOUGHT and r1 < RSI_OVERBOUGHT) or (r1 >= RSI_EXTREME))
    ...
    try:
        ...
        natr_series = (atr_series / close.replace(0, np.nan)) * 100.0
    ...
    sell_flags = {
        ...
        "ext_dev200": sell_ext_dev200,
        "di_bear": sell_di_bear,
        "vol_distribution": sell_vol_distribution,
        "natr_spike": sell_natr_spike,
    }

    # 8点満点に合わせて（※ext_dev200 と natr_spike を両方入れると9点になるので調整）
    # 優先度：natr_spike を採用し、ext_dev200 は「理由」列に残すが点には入れない（好みで変更可）
...
def apply_filters_and_make_trades(all_df: pd.DataFrame, val_df: pd.DataFrame, end_ts):
    ...
    if SELL_EXIT_BEFORE_EARNINGS and "days_to_earnings" in merged.columns:
        ...
        merged.loc[earn_exit, "sell_reason"] = np.where(sr.eq(""), "before_earnings", sr + ";before_earnings")
...
if __name__ == "__main__":
    ...
    # ★ここで all_rows2 / buy_rows / sell_rows を作る
    try:
    ...
    sell_rows.to_csv(f"screen_{as_of_str}_sell.csv", index=False, encoding="utf-8-sig")  # ★追加

    print("as_of:", as_of_str, "total:", len(all_rows2), "buy:", len(buy_rows), "sell:", len(sell_rows))
    if not sell_rows.empty:
        print(sell_rows[["ticker","sell_score","sell_reason","close","pullback_20d","dev200","rsi","adx14","days_to_earnings"]]
