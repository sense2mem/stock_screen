...
def load_signals(signals_dir: Path | str, pattern: str, score_min: float) -> tuple[pd.DataFrame, int, int]:
    ...
    frames, raw_count, screening_dates = [], 0, []
    for path in paths:
        try:
        ...
        frame = frame[frame["ticker"].notna() & frame["ticker"].ne("") & frame["score"].ge(score_min)]
        frames.append(frame)
    if not frames:
    ...
    signals = signals.sort_values(["signal_date", "ticker"]).drop_duplicates(["signal_date", "ticker"], keep="first")
    for column in OPTIONAL_COLUMNS:
        if column not in signals:
            signals[column] = pd.NA
    signals = signals.reset_index(drop=True)
    # Keep even dates on which no ticker passed score_min.  Such a date breaks a
...
def select_signals(signals: pd.DataFrame, mode: str) -> pd.DataFrame:
    """Mark signals used; streaks are based on the ordered set of screening dates."""
    result = signals.copy()
    result["is_used"] = True
    result["excluded_reason"] = ""
    if mode == "all" or result.empty:
        return result
    dates = signals.attrs.get("screening_dates") or pd.to_datetime(result["signal_date"]).dt.normalize().unique()
...
def _normalize_prices(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    if isinstance(frame.columns, pd.MultiIndex):
        # A per-ticker frame may retain either MultiIndex orientation.
...
def evaluate_signal(signal: pd.Series, prices: pd.DataFrame, holdings: Iterable[int]) -> list[dict]:
    """Evaluate one signal against injected OHLC prices (no network dependency)."""
    prices = _normalize_prices(prices)
    signal_date = pd.Timestamp(signal["signal_date"]).tz_localize(None).normalize()
    base = {c: signal.get(c, pd.NA) for c in ["signal_date", "ticker", "name", "score", "sell_score"] + OPTIONAL_COLUMNS[2:]}
    future = prices.loc[prices.index > signal_date]
    rows = []
    for holding in holdings:
        row = {**base, "holding_days": int(holding), "entry_date": pd.NaT, "entry_price": np.nan,
               "exit_date": pd.NaT, "exit_price": np.nan, "return_pct": np.nan,
               "mfe_pct": np.nan, "mae_pct": np.nan, "status": "INVALID_SIGNAL"}
        if future.empty:
            row["status"] = "NO_NEXT_TRADING_DAY"
        elif not {"Open", "High", "Low", "Close"}.issubset(prices.columns):
            ...
            if pd.isna(entry) or entry <= 0:
                ...
                row["entry_date"], row["entry_price"] = future.index[0], float(entry)
                if len(future) < holding:
                    ...
                    if pd.isna(exit_price) or highs.dropna().empty or lows.dropna().empty:
                        ...
                        row.update({
                            "exit_date": window.index[-1], "exit_price": float(exit_price),
                            "return_pct": (exit_price / entry - 1) * 100,
                            "mfe_pct": (highs.max() / entry - 1) * 100,
                            "mae_pct": (lows.min() / entry - 1) * 100, "status": "CLOSED",
                        })
        rows.append(row)
    return rows
...
def summarize_results(detail: pd.DataFrame, holdings: Iterable[int]) -> pd.DataFrame:
    ...
    for holding in holdings:
        ...
        returns = pd.to_numeric(group.get("return_pct", pd.Series(dtype=float)), errors="coerce").dropna()
        wins, losses = returns[returns > 0], returns[returns < 0]
        loss_sum = -losses.sum()
        rows.append({
            "holding_days": holding, "trades": len(returns), "wins": len(wins), "losses": len(losses),
            "win_rate_pct": len(wins) / len(returns) * 100 if len(returns) else np.nan,
            "average_return_pct": returns.mean(), "median_return_pct": returns.median(),
            "average_win_pct": wins.mean(), "average_loss_pct": losses.mean(),
            "profit_factor": wins.sum() / loss_sum if loss_sum > 0 else np.nan,
            "best_return_pct": returns.max(), "worst_return_pct": returns.min(),
            "average_mfe_pct": group["mfe_pct"].mean(), "median_mfe_pct": group["mfe_pct"].median(),
            "average_mae_pct": group["mae_pct"].mean(), "median_mae_pct": group["mae_pct"].median(),
        })
    return pd.DataFrame(rows, columns=SUMMARY_COLUMNS)


def download_prices(tickers: list[str], start: pd.Timestamp, end: pd.Timestamp, chunk_size: int = 20) -> tuple[dict[str, pd.DataFrame], dict[str, str]]:
    prices, failures = {}, {}
    for offset in range(0, len(tickers), chunk_size):
        chunk = tickers[offset:offset + chunk_size]
        try:
            data = yf.download(chunk, start=start.strftime("%Y-%m-%d"), end=end.strftime("%Y-%m-%d"),
        ...
        for ticker in chunk:
            try:
                frame = data[ticker] if isinstance(data.columns, pd.MultiIndex) and ticker in data.columns.get_level_values(0) else data
...
def run(args: argparse.Namespace) -> None:
    ...
    LOGGER.info("CSV files=%d raw signals=%d score>=%.2f unique signals=%d", file_count, raw_count, args.score_min, len(signals))
    LOGGER.info("signals after %s=%d tickers=%d", args.signal_mode, len(used), len(tickers))
    ...
    for _, signal in used.iterrows():
        ...
        if ticker not in prices:
            ...
            for holding in args.holdings:
                ...
                row.update({"holding_days": holding, "status": "PRICE_DOWNLOAD_FAILED"})
                detail_rows.append(row)
                status_rows.append({"signal_date": signal["signal_date"], "ticker": ticker, "holding_days": holding,
                                    "status": "PRICE_DOWNLOAD_FAILED", "message": message})
            continue
        for row in evaluate_signal(signal, prices[ticker], args.holdings):
            detail_rows.append(row)
            if row["status"] != "CLOSED":
                status_rows.append({"signal_date": row["signal_date"], "ticker": ticker,
                                    "holding_days": row["holding_days"], "status": row["status"],
    ...
    for holding in args.holdings:
        LOGGER.info("holding=%d CLOSED=%d OPEN=%d", holding,
                    int(((detail.holding_days == holding) & (detail.status == "CLOSED")).sum()),
                    int(((detail.holding_days == holding) & (detail.status == "OPEN_INSUFFICIENT_DAYS")).sum()))
